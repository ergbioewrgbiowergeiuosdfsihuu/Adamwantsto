...
const LocalVideoTile = ({ stream, isVideoOff, isMuted, currentUser, localVideoRef }: { stream: MediaStream | null, isVideoOff: boolean, isMuted: boolean, currentUser: UserProfile | null, localVideoRef: React.RefObject<HTMLVideoElement> }) => {
  ...
  useEffect(() => {
    if (localVideoRef.current && stream && !isVideoOff) {
      ...
      localVideoRef.current.play().catch(e => { /* ignore */ });
    }
...
const RemoteVideo = ({ stream, user, audioDeviceId }: { stream: MediaStream, user?: UserProfile, key?: React.Key, audioDeviceId?: string | null }) => {
  ...
  useEffect(() => {
    const updateCount = () => {
      ...
      // Force video element to reload its source object when tracks change
  ...
  // Poll for muted state changes (WebRTC doesn't reliably fire events for this on all browsers)
  const [isMuted, setIsMuted] = useState(stream.getVideoTracks().every(t => t.muted) || videoTrackCount === 0);
  useEffect(() => {
    const interval = setInterval(() => {
      setIsMuted(stream.getVideoTracks().every(t => t.muted) || stream.getVideoTracks().length === 0);
    }, 500);
    return () => clearInterval(interval);
  }, [stream]);

  const audioOnly = videoTrackCount === 0 || isMuted;
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      ref.current.play().catch(e => { /* ignore */ });
    }
...
const preferVP8 = (sdp: string) => {
  ...
  lines.forEach(line => {
    const match = line.match(/^a=rtpmap:(\d+) VP8\//i);
  ...
  const mLinePayloads = parts.slice(3);

  const reorderedPayloads = [...vp8Payloads];
  mLinePayloads.forEach(pt => {
    if (!vp8Payloads.includes(pt)) reorderedPayloads.push(pt);
  });

  lines[mLineIndex] = parts.slice(0, 3).join(' ') + ' ' + reorderedPayloads.join(' ');
  return lines.join('\r\n');
};
  ...
  const createPeerConnection = (targetUserId: string, isInitiator: boolean) => {
    ...
    peer.onnegotiationneeded = async () => {
      try {
        ...
        if (offer.sdp) offer.sdp = preferVP8(offer.sdp);
    ...
    // Pre-warm transceivers. This prevents needing renegotiation when we later add/replace tracks
    const audioTrack = mediaStreamRef.current?.getAudioTracks()[0];
    const videoTrack = mediaStreamRef.current?.getVideoTracks()[0];

    peer.addTransceiver(audioTrack || 'audio', { direction: 'sendrecv', streams: mediaStreamRef.current ? [mediaStreamRef.current] : [] });
    peer.addTransceiver(videoTrack || 'video', { direction: 'sendrecv', streams: mediaStreamRef.current ? [mediaStreamRef.current] : [] });
  ...
  useEffect(() => {
    if (callState === 'active' || callState === 'calling') {
      callParticipants.forEach(p => {
        if (p !== currentUser?.id && !peersRef.current.has(p)) {
          createPeerConnection(p, true);
        }
      });
    }
  }, [callParticipants, callState, currentUser]);
      ...
      try {
        ...
        if (description) {
          ...
          if (description.type === 'offer') {
            ...
            if (answer.sdp) answer.sdp = preferVP8(answer.sdp);
  ...
  const replaceVideoTrack = async (type: 'video' | 'screen') => {
    ...
    try {
      ...
      if (newVideoTrack) {
        ...
        // Safely replace track using the existing video transceiver
        peersRef.current.forEach(peer => {
          const videoTransceiver = peer.getTransceivers().find(t => t.receiver.track?.kind === 'video' || t.sender.track?.kind === 'video');
          
          if (videoTransceiver && videoTransceiver.sender) {
            videoTransceiver.sender.replaceTrack(newVideoTrack).catch(console.error);
            if (videoTransceiver.direction !== 'sendrecv' && videoTransceiver.direction !== 'sendonly') {
              videoTransceiver.direction = 'sendrecv';
            }
          } else {
            // Fallback only if no transceiver exists
